...
This is the canonical repository map. Active Development contains preserved earlier lines, the current v21 line, and files required to run, understand, and validate them.

本檔是正式 repository 地圖。作用中的 Development 保留較早版本與目前 v21，以及執行、理解與驗證所需的檔案。
...
| `Apps/Standalone/` | Immutable v15–v21 HTML releases plus v16 runtime assets. / 不可變 v15–v21 HTML 與 v16 runtime assets。 |
...
| `Apps/Public-Web/v4/` | Current immutable v21 GitHub Pages release. / 目前不可變 v21 GitHub Pages release。 |
...
| `Development/` | Current v21 source, automation, validation, and product contracts. / 目前 v21 source、自動化、驗證與產品合約。 |
...
| `Open Truth and Dare.cmd` | One-click Windows v21 launcher. / Windows v21 一鍵啟動器。 |
...
| `Development/Source/Main-App-v21/` | Current source with 18 independent hidden-Taiwan hotspots, 600 ms reveal, blinking locator, and the preserved v20 visual contract. / 目前 source，含 18 個獨立藏台灣熱點、600ms 揭示、閃爍定位與保留的 v20 視覺合約。 |
...
| `Development/Automation/Scripts/` | Versioned v18–v21 PWA, standalone, and public-release builders. / 版本化 v18–v21 建置器。 |
| `Development/Automation/Tools/` | Loopback-only v21 desktop server helper. / 僅限本機的 v21 desktop server。 |
| `Development/Tests/` | v18–v21, structure, and repository validators. / v18–v21、結構與 repository 驗證。 |
...
# Restore dependencies and work on current v21 source
...
Existing release outputs are immutable. New product behavior after v21 requires v22. Uncertain or proposed-deletion content must move to `_pending/` and be indexed; never permanently delete it without explicit human approval.

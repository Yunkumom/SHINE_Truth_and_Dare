...
import { DEFAULT_PRESENTATION, presentationForArtwork } from '../presentation/presentation-model'
import type { PresentationDocument } from '../presentation/presentation-model'
import { adjustedPortraitFocus, calculateZoomedCoverPlacement } from './portrait-focus'
import { artworkTitle } from './artwork-copy'
...
function drawImageCover(context: CanvasRenderingContext2D, image: HTMLImageElement, x: number, y: number, width: number, height: number, focus: { x: number, y: number }, zoom: number) {
  const placement = calculateZoomedCoverPlacement({ width: image.naturalWidth, height: image.naturalHeight }, focus, { x, y, width, height }, zoom)
...
export async function createCardPng(composition: EncounterComposition, language: Language, participants: ParticipantExchange[] = [], layout: LayoutDocument = DEFAULT_LAYOUT, presentation: PresentationDocument = DEFAULT_PRESENTATION, notes: { answerKeywords?: string } = {}): Promise<File> {
  ...
  const artworkPresentation = presentationForArtwork(presentation, artwork.id)
  const artworkFocus = adjustedPortraitFocus(artwork.portraitFocus, artworkPresentation)
  ...
  context.fillStyle = '#8d4d32'; context.font = '700 24px system-ui'; context.fillText('今天的題目 · TODAY’S QUESTION', question.x + 38, question.y + 43)
  ...
  drawPanelText(context, lines.question, question.x + 38, question.y + 103, question.width - 76, `600 ${37 * presentation.question.fontScale}px serif`, 46 * presentation.question.fontScale)
  if (notes.answerKeywords) {
    context.fillStyle = '#8d4d32'; context.font = '600 22px system-ui'
    context.fillText(`回答關鍵字 · ${notes.answerKeywords}`, question.x + 38, question.y + question.height - 28)
  }

  const blessingScale = KEEPSAKE_CANVAS.width / 430
  const blessingStyle = presentation.blessing
  const blessingX = blessing.x + blessingStyle.offsetX * blessingScale
  const blessingY = blessing.y + blessingStyle.offsetY * blessingScale
  ...
  return new File([blob], `encounter-card-V47-${card.id}-${artwork.id}.png`, { type: 'image/png' })
}
...
export async function shareOrDownload(composition: EncounterComposition, language: Language, participants: ParticipantExchange[] = [], layout: LayoutDocument = DEFAULT_LAYOUT, presentation: PresentationDocument = DEFAULT_PRESENTATION, notes: { answerKeywords?: string } = {}): Promise<'shared' | 'downloaded' | 'cancelled'> {
  return deliverCardFile(await createCardPng(composition, language, participants, layout, presentation, notes))
}

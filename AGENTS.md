...
- v39 authored source remains under `Development/Source/Main-App-v39/src/`; its verified `dist/` is generated and immutable.
- Product behavior changes after v39 require a new versioned source and output line.
- The current Windows launcher must serve `Apps/Standalone/encounter_cards_v39.html` through `Development/Automation/Tools/serve_truth_and_dare.ps1`; direct `file://` execution is unsupported for the complete contract.
...
- v39 人工 source 位於 `Development/Source/Main-App-v39/src/`；已驗證 `dist/` 為不可變生成成品。
- v39 後的產品行為修改必須建立新版本。
- Windows 啟動器必須透過本機 server 開啟 v39；完整合約不支援直接 `file://`。
...
1. Complete physical iPhone v39 regression across full-width mode selection, direct keepsake, settings, the 42-card library, artwork adjustment, draw/flip, PNG, Share Sheet, installation, and offline update.

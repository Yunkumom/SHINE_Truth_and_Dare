...
- Treat `Apps/Standalone/encounter_cards_v15.html` through `encounter_cards_v35.html` as immutable releases. Never overwrite or hand-edit them.
- v35 authored source remains under `Development/Source/Main-App-v35/src/`; its verified `dist/` is generated and immutable.
- Product behavior changes after v35 require a new versioned source and output line.
- The current Windows launcher must serve `Apps/Standalone/encounter_cards_v35.html` through `Development/Automation/Tools/serve_truth_and_dare.ps1`; direct `file://` execution is unsupported for the complete contract.
...
- 將 `Apps/Standalone/` 中 v15–v35 視為不可變 release，不得覆寫或手改。
- v35 人工 source 位於 `Development/Source/Main-App-v35/src/`；已驗證 `dist/` 為不可變生成成品。
- v35 後的產品行為修改必須建立新版本。
- Windows 啟動器必須透過本機 server 開啟 v35；完整合約不支援直接 `file://`。
...
1. Complete physical iPhone v35 regression across the 42-card library, Taiwan alignment, exact choice, library swipe, draw/flip, PNG, Share Sheet, installation, and offline update.

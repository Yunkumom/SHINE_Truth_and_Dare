...
$pendingOutput = '_pending/Apps-Public-Web_2026-07-19'
$required = @('Development/Source/Public-Web/v1/package.json',"$pendingOutput/index.html","$pendingOutput/v1/index.html","$pendingOutput/v1/manifest.json",'.github/workflows/pages.yml')
foreach ($item in $required) { if (-not (Test-Path -LiteralPath (Join-Path $root $item))) { throw "Missing public v1 file: $item" } }
$manifest = Get-Content -LiteralPath (Join-Path $root "$pendingOutput/v1/manifest.json") -Raw | ConvertFrom-Json
if ($manifest.version -ne 'v1' -or $manifest.immutable -ne $true) { throw 'Public manifest is not immutable v1' }
foreach ($entry in $manifest.files.PSObject.Properties) {
  $file = Join-Path (Join-Path $root "$pendingOutput/v1") $entry.Name
...
$ownedCards = Get-ChildItem -LiteralPath (Join-Path $root "$pendingOutput/v1/assets/cards") -Filter '*.webp' -File | Where-Object Name -ne 'fallback.webp'
...
$publicFiles = Get-ChildItem -LiteralPath (Join-Path $root $pendingOutput) -Recurse -File
if ($publicFiles | Where-Object Name -Match '(?i)(\.env|secret|credential|private)') { throw "Secret-like filename found under $pendingOutput/" }

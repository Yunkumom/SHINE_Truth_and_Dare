...
Create `tools/build_v16.ps1`. It reads v15, verifies its canonical SHA-256, extracts the five embedded modules into hierarchical HTTP assets, changes HTML and RSC viewport metadata plus visible version labels, broadens the existing mobile layout rules to become the shared layout, appends safe-area and desktop phone-frame CSS inside the existing style node, and writes v16 only when no conflicting output exists.
...
The initial embedded-module build failed with a non-hierarchical module-base error. The next build extracted the modules but added document nodes that caused React hydration error #418. Both failed intermediates were moved to `_pending/` and indexed. The final implementation preserves React's document structure and uses extracted HTTP modules.

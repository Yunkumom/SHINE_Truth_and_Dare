  ...
  return <section className="artwork-editor artwork-position-editor card-layout-adjuster" role="dialog" aria-modal="true" aria-label="調整卡片 · Adjust card">
    ...
    <div className="card-adjust-live-shell">
      <article className={`mythic-card card-adjust-live-preview${tab === 'layout' ? ' is-layout-guided' : ''}`} style={cardStyle} data-testid="card-adjust-live-preview">
        <div className="mythic-card-header" data-adjust-box="標題"><div><span>護行之卡 · ENCOUNTER GUARDIAN</span><h2>{artwork.zhName}</h2><small>{artwork.enName}</small></div><i>✦</i></div>
        ...
        <div className="mythic-text-panel" data-adjust-box="文字">
          {manager.showRealYou && <div className="mythic-prompt-meta" data-adjust-box="真正的你"><b>真正的你 · THE REAL YOU</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div>}
          ...
          <div className="mythic-blessing" data-adjust-box="祝福"><span>給這次相遇的祝福 · BLESSING</span>{language !== 'en' && <p lang="zh-Hant">{encounter.blessing.zh}</p>}{language !== 'zh' && <small className={language === 'en' ? 'english-primary' : ''} lang="en">{encounter.blessing.en}</small>}</div>
        </div>
      </article>
    </div>

...
export const DESKTOP_WORKSPACE_CHROME = 60

export type ViewportSize = { width: number, height: number }

export function calculatePhoneScale({ width, height }: ViewportSize): number {
  if (width < DESKTOP_BREAKPOINT) return Math.min(1, Math.max(0, width) / PHONE_WIDTH, Math.max(0, height) / PHONE_HEIGHT)
...
export function calculateDesktopScales(height: number) {
  const availableScale = Math.max(0, height - DESKTOP_WORKSPACE_CHROME) / PHONE_HEIGHT
  return {
    workbench: Math.min(.76, availableScale),
    preview: Math.min(.62, availableScale),
  }
}

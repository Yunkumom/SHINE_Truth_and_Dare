...
export function adjustedPortraitFocus(focus: PortraitFocus, adjustment: { offsetX: number, offsetY: number }): PortraitFocus {
  return { x: clamp(focus.x + adjustment.offsetX, 0, 100), y: clamp(focus.y + adjustment.offsetY, 0, 100) }
}
...
export function calculateZoomedCoverPlacement(source: Size, focus: PortraitFocus, target: Rect, zoom = 1) {
  const base = calculateCoverPlacement(source, focus, target)
  const safeZoom = clamp(zoom, 1, 2.4)
  const width = base.width * safeZoom
  const height = base.height * safeZoom
  const desiredX = target.x + target.width / 2 - width * clamp(focus.x, 0, 100) / 100
  const desiredY = target.y + target.height / 2 - height * clamp(focus.y, 0, 100) / 100
  return { x: clamp(desiredX, target.x + target.width - width, target.x), y: clamp(desiredY, target.y + target.height - height, target.y), width, height, scale: base.scale * safeZoom }
}

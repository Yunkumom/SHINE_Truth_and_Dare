...
$images = @(Get-ChildItem -LiteralPath (Join-Path $projectRoot 'Assets/Deities') -Filter '*.png' -File | Where-Object { $_.Name -notmatch '-v\d+\.png$' })
...
Assert-V19Condition (Test-Path -LiteralPath (Join-Path $projectRoot 'Apps/Public-Web/v2/index.html') -PathType Leaf) 'Preserved Public Web v2 remains available for v19'

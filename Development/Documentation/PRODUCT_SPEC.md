...
## Current v46 Product Surface / 目前 v46 產品介面

- `/` offers mobile and desktop-studio entry choices.
- `/mobile` hosts the complete touch-first encounter experience; `/studio` frames the same product in standard and large phone simulations.
- `/v46/index.html` is the generated PWA surface with 42 governed artworks, 62 SHINE question-book records, Level 1–5, Truth/Dare/Surprise, exact selection, card library, artwork adjustment, direct keepsake, PNG share/download, and offline caching.
- v46 preserves Chinese, English, bilingual, consent, mandatory blessing, memory-only personal input, and non-personal local preference contracts.
- v46 has no active accounts, authentication, analytics, D1/R2, or backend persistence. v39 remains the latest immutable standalone fallback.

- `/` 提供手機與電腦工作室入口。
- `/mobile` 承載完整觸控優先 encounter 體驗；`/studio` 以標準及大型手機框模擬相同產品。
- `/v46/index.html` 是生成的 PWA 介面，包含 42 張受治理 artwork、62 筆 SHINE 題庫、Level 1–5、真心話／小挑戰／隨機、指定選擇、卡庫、圖片調整、直接紀念卡、PNG 分享／下載與離線快取。
- v46 保留中文、英文、雙語、同意、必備祝福、個資僅存記憶體及非個人本機偏好合約。
- v46 沒有作用中的帳號、authentication、analytics、D1／R2 或 backend persistence；v39 保留為最新不可變 standalone 備援。

...
v20 keeps those 60 prompts but no longer permanently binds a prompt to one deity image. v21 adds one independent hidden-Taiwan hotspot to every artwork. v22 adds optional participant exchange rows. v23 replaces all 18 images with Taiwan-safe edits whose precise coloured silhouettes remain inside the visible crop and share one canonical reveal outline. A session independently selects one eligible question, one artwork, and one of 12 bilingual blessings; the chosen trio remains stable for the visible card and its exported PNG. Every final commemorative card must contain a blessing, omitted participant data must not be drawn, and editor/runtime locator chrome must not be baked into the exported keepsake.

...
v20 keeps those 60 prompts but no longer permanently binds a prompt to one deity image. v21 preserves that composition and adds one independent hidden-Taiwan hotspot to every artwork. A session independently selects one eligible question, one of 18 deity artworks, and one of 12 bilingual blessings; the chosen trio remains stable for the visible card and its exported PNG. Every final commemorative card must contain a blessing, and the runtime locator must not be baked into the exported keepsake.

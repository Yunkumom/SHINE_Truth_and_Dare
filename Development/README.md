...
The active workspace repository is `products/truth-or-dare` (Truth or Dare / 真心話大冒險). Legacy launcher, script, GitHub, and immutable release identifiers that contain `truth_and_dare` or `Truth and Dare` remain unchanged for compatibility.
目前作用中的 workspace repository 是 `products/truth-or-dare`（Truth or Dare／真心話大冒險）。為維持相容性，包含 `truth_and_dare` 或 `Truth and Dare` 的舊版 launcher、script、GitHub 與不可變 release 識別碼維持不變。

This directory contains preserved v18–v39 source lines and the current Encounter Cards v46 Sites/Vinext source line. Historical v17 source, older completed plans, imported packages, old-version builders, and generated dependencies remain recoverable under `../_pending/`.

此目錄保留 v18–v39 source 與目前作用中的 Encounter Cards v46 Sites／Vinext source。v17 歷史 source、舊版計畫、匯入套件、builder 與可重建依賴仍可由 `../_pending/` 回復。
...
├── Source/Main-App-v39/       # Preserved full-width mobile modes and standalone fallback
├── Source/Main-App-v46/       # Current Sites/Vinext app and generated v46 PWA
...
├── Tests/                     # Current v46, structure, and preserved-release contracts
...
- `Source/Main-App-v34/` — preserved immutable React/TypeScript line; its generated `dist/` is the verified v34 PWA output.
...
## v35 Entrance Card Library / v35 入口卡庫

- `Source/Main-App-v35/` — current authored React/TypeScript line and verified v35 PWA output.
- `Source/Main-App-v35/src/components/CardLibrary.tsx` — accessible modal library with collection filters, wrap-around navigation, pointer swipe, arrows, and keyboard controls.
- `Source/Main-App-v35/src/lib/card-library.ts` — pure navigation, filtering, and swipe policies.
- `Source/Main-App-v35/src/styles/v35.css` — 430 × 932 library layout, full-card presentation, and reduced-motion rules.
- `Source/Public-Web/v18/README.md` — v35 to Public Web v18 release recipe.
- `Automation/Scripts/finalize-pwa-v35.mjs` — finalizes the v35 offline precache.
...
- `Source/Main-App-v39/` — preserved authored React/TypeScript line and verified v39 PWA output; latest immutable standalone fallback.
...
## Current v46 Sites/Vinext Application / 目前 v46 Sites／Vinext 應用

- `Source/Main-App-v46/` — current governed multi-route source and verified Sites build.
- `Source/Main-App-v46/app/` — `/`, `/mobile`, and `/studio` wrapper routes plus the authored encounter application.
- `Source/Main-App-v46/app/encounter/` — privacy-first bilingual game source with 42 governed artworks and the 62-question SHINE book.
- `Source/Main-App-v46/public/v46/` — generated versioned encounter PWA with a complete 50-URL service-worker precache.
- `Source/Main-App-v46/dist/` — generated verified Sites Worker artifact.
- `Source/Main-App-v46/scripts/finalize-encounter.mjs` — inventories the generated v46 browser artifact and finalizes its precache.
- `Tests/validate_v46.mjs` — verifies v46 identity, routes, privacy capability boundary, artwork/question inventory, generated PWA, and preserved intake.
- `_pending/v46-generated-development-state_2026-08-16/` — recoverable dependencies and build caches moved out of active Development after final verification.
- `_pending/SHINE_Truth_and_Dare_v46_GitHub_Lite_2026-08-16/` — byte-preserved original intake with mixed v40/v41/v46 provenance.

- `Source/Main-App-v46/` — 目前受治理的多路由 source 與已驗證 Sites build。
- `Source/Main-App-v46/app/` — `/`、`/mobile`、`/studio` wrapper routes 與人工維護 encounter 應用。
- `Source/Main-App-v46/app/encounter/` — 隱私優先雙語遊戲 source，含 42 張受治理 artwork 與 62 題 SHINE 題庫。
- `Source/Main-App-v46/public/v46/` — 生成的版本化 encounter PWA，含完整 50 URL service-worker precache。
- `Source/Main-App-v46/dist/` — 生成且已驗證的 Sites Worker artifact。
- `Source/Main-App-v46/scripts/finalize-encounter.mjs` — 列出生成的 v46 browser artifact 並完成 precache。
- `Tests/validate_v46.mjs` — 驗證 v46 識別、routes、隱私能力邊界、artwork／question inventory、生成 PWA 與保存 intake。
- `_pending/v46-generated-development-state_2026-08-16/` — 最終驗證後從作用中 Development 移出的可回復依賴與建置 cache。
- `_pending/SHINE_Truth_and_Dare_v46_GitHub_Lite_2026-08-16/` — 逐位元保存、含混合 v40／v41／v46 來源紀錄的原始 intake。

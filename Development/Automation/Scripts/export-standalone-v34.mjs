...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v34', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v34.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V34"><!-- Standalone v34 adds explicit collection versions and Taiwan Zodiac Local Stories. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v34 already exists; create the next version instead of overwriting a release')
} catch (error) {

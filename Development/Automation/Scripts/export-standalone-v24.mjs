...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v24', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v24.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V24"><!-- Standalone v24 uses a precise Natural Earth-derived Taiwan coastline with a transparent dual-stroke glow. --></head>')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v24 already exists; create the next version instead of overwriting a release')
} catch (error) {

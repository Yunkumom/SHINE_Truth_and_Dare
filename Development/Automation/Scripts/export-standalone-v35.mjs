...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v35', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v35.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V35"><!-- Standalone v35 adds the complete swipeable entrance card library. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v35 already exists; create the next version instead of overwriting a release')
} catch (error) {

...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v22', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v22.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V22"><!-- Standalone v22 embeds the layout editor, swipe deck, 18 deity artworks, hidden-Taiwan locators, contact-selectable keepsakes, and blessings. --></head>')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v22 already exists; create the next version instead of overwriting a release')
} catch (error) {

...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v30', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v30.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V30"><!-- Standalone v30 uses a precise Natural Earth-derived Taiwan coastline with a transparent dual-stroke glow. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v30 already exists; create the next version instead of overwriting a release')
} catch (error) {

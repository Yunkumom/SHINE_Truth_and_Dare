...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v39', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v39.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V39"><!-- Standalone v39 adds full-width mobile reflow, mode selection, and direct keepsake creation. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v39 already exists; create the next version instead of overwriting a release')
} catch (error) {

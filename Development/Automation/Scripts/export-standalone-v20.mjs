...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v20', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v20.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V20"><!-- Standalone v20 embeds runtime code, 18 deity artworks, and blessings. --></head>')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v20 already exists; create the next version instead of overwriting a release')
} catch (error) {

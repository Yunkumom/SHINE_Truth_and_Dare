...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v38', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v38.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V38"><!-- Standalone v38 adds one-viewport mobile fit and reliable grid-guided artwork adjustment. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v38 already exists; create the next version instead of overwriting a release')
} catch (error) {

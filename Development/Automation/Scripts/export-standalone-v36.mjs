...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v36', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v36.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V36"><!-- Standalone v36 integrates the mobile ChatGPT Work settings, session question controls, artwork selection, and artwork adjustment. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v36 already exists; create the next version instead of overwriting a release')
} catch (error) {

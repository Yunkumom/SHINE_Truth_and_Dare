...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v21', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v21.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V21"><!-- Standalone v21 embeds runtime code, 18 deity artworks, hidden-Taiwan locators, and blessings. --></head>')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v21 already exists; create the next version instead of overwriting a release')
} catch (error) {

...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v33', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v33.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V33"><!-- Standalone v33 adds governed Taiwan zodiac art plus independent exact artwork and question choices. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v33 already exists; create the next version instead of overwriting a release')
} catch (error) {

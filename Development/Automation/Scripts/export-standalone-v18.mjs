...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v18', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v18.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V18"><!-- Standalone v18: all runtime CSS and JavaScript are embedded. --></head>')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v18 already exists; create v19 instead of overwriting a release')
} catch (error) {

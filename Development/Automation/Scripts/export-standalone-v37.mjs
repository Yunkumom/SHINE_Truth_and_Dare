...
const dist = join(projectRoot, 'Development', 'Source', 'Main-App-v37', 'dist')
const output = join(projectRoot, 'Apps', 'Standalone', 'encounter_cards_v37.html')
...
html = html.replace('</head>', '<meta name="encounter-release" content="V37"><!-- Standalone v37 adds the compact fixed three-column desktop studio and interactive phone Test mode. --></head>')
html = html.replaceAll('\r\n', '\n')
try {
  const existing = await readFile(output, 'utf8')
  if (existing !== html) throw new Error('A different standalone v37 already exists; create the next version instead of overwriting a release')
} catch (error) {

...
import type { ArtworkPreference, Card, EncounterComposition, Language, Level, Mode, ParticipantExchange } from './types'
...
export default function App() {
  ...
  const [desktopScales, setDesktopScales] = useState(() => ({ workbench: Math.min(.88, (window.innerHeight - 48) / 932), preview: Math.min(.76, (window.innerHeight - 48) / 932) }))
  ...
  const [candidateArtworks, setCandidateArtworks] = useState(() => createArtworkCandidates(ARTWORK_COLLECTIONS, DEITY_ART, { mode: 'random', collectionId: DEFAULT_COLLECTION_ID }))
  ...
  useEffect(() => {
    const updateScale = () => { setPhoneScale(calculatePhoneScale({ width: window.innerWidth, height: window.innerHeight })); setDesktopWorkspace(window.innerWidth >= 1100); setDesktopScales({ workbench: Math.min(.88, (window.innerHeight - 48) / 932), preview: Math.min(.76, (window.innerHeight - 48) / 932) }) }
  ...
  function draw(artworkId?: string) {
    ...
    const artwork = artworkId
      ? resolvePreferredArtwork({ mode: 'specific', collectionId: artworkPreference.collectionId, artworkId }, ARTWORK_COLLECTIONS, DEITY_ART, current?.artwork.id)
      : resolvePreferredArtwork(artworkPreference, ARTWORK_COLLECTIONS, DEITY_ART, current?.artwork.id)
  ...
  function goHome() { setPlaying(false); setKeepsakeOpen(false); if (desktopMode === 'settings') { setEditorScreen('setup'); setSelectedBlock('hero') } }
  ...
  function header(screen: 'setup' | 'game') {
    return block(screen, 'header', <header className="site-header"><button className="wordmark" type="button" onClick={goHome} aria-label="Encounter Cards home"><span className="mark">✦</span><span>相遇卡 <small>ENCOUNTER CARDS · V32</small></span></button><div className="header-tools">{playing && <div className="font-size-control" aria-label="Font size"><button onClick={() => changeFont(-.1)}>A−</button><button onClick={() => changeFont(.1)}>A＋</button></div>}<div className="language-switch" role="group" aria-label="Language 語言">{([['zh', '中'], ['en', 'EN'], ['bilingual', '中/EN']] as const).map(([value, label]) => <button key={value} className={language === value ? 'active' : ''} aria-pressed={language === value} aria-label={value === 'zh' ? '中文' : value === 'en' ? 'English' : '雙語 Bilingual'} onClick={() => chooseLanguage(value)}>{label}</button>)}</div></div></header>, 'header-block')
  }
  ...
  const setupScreen = <section className="screen-canvas setup-canvas" aria-label="開始遊戲設定 · Game setup">
    ...
    <div className="advanced-deck-choice" data-no-layout-drag>
      <button type="button" className="advanced-deck-disclosure" aria-expanded={advancedDeckOpen} aria-controls="advanced-deck-panel" aria-label="進階選牌 · Advanced deck choice" onClick={() => setAdvancedDeckOpen(open => !open)}><span>進階選牌 · Advanced deck choice</span><b>{artworkPreference.mode === 'specific' ? DEITY_ART.find(item => item.id === artworkPreference.artworkId)?.zhName : '全部隨機'} {advancedDeckOpen ? '−' : '+'}</b></button>
      {advancedDeckOpen && <div id="advanced-deck-panel" className="advanced-deck-panel" role="dialog" aria-label="Advanced deck choice panel">
        <div className="advanced-deck-panel-heading"><div><b>選擇牌組與卡面</b><small>COLLECTION &amp; ARTWORK</small></div><button type="button" aria-label="Close advanced deck choice" onClick={() => setAdvancedDeckOpen(false)}>×</button></div>
        <label className="random-artwork-option"><input type="radio" name="artwork-choice" aria-label="全部隨機 · Random artwork" checked={artworkPreference.mode === 'random'} onChange={() => setArtworkPreference({ mode: 'random', collectionId: DEFAULT_COLLECTION_ID })} />全部隨機 · Random artwork</label>
        <div className="collection-roadmap">{ARTWORK_COLLECTIONS.map(collection => <article key={collection.id} className={collection.availability === 'planned' ? 'is-planned' : 'is-available'}><b>{collection.zhName} · {collection.enName}</b><span>{collection.availability === 'planned' ? '規劃中 · Planned' : collection.zhDescription}</span></article>)}</div>
        <div className="artwork-picker" aria-label="Taiwan deity artwork choices">{DEITY_ART.map(artwork => <button type="button" key={artwork.id} aria-label={`指定 ${artwork.zhName} · ${artwork.id}`} aria-pressed={artworkPreference.mode === 'specific' && artworkPreference.artworkId === artwork.id} onClick={() => setArtworkPreference({ mode: 'specific', collectionId: DEFAULT_COLLECTION_ID, artworkId: artwork.id })}><img src={artwork.src} alt="" /><span>{artwork.zhName}</span></button>)}</div>
      </div>}
    </div>
    {block('setup', 'begin', <div className="begin-dock"><button className="primary-button" aria-label={t.begin} onClick={() => { setCandidateArtworks(createArtworkCandidates(ARTWORK_COLLECTIONS, DEITY_ART, artworkPreference)); setRevealed(false); setPlaying(true); setKeepsakeOpen(false); setAdvancedDeckOpen(false); setEditorScreen('game'); setSelectedBlock('card') }}><span>{t.begin}</span><small>LEVEL {level} · V32 · {levelLabels[level - 1][1].toUpperCase()}</small><i>→</i></button><p className="install-note">{offlineReady ? '✓ Offline ready / 可離線使用' : t.install}</p></div>, 'setup-begin-block')}
  ...
  const gameScreen = <section className="screen-canvas game-canvas">
    ...
    {block('game', 'card', <SwipeDeck key={current ? `${current.card.id}-${current.artwork.id}` : 'deck'} revealed={editingActive || revealed} onDraw={draw}><div className="card-face card-back"><span className="ornament">✦</span><p>ENCOUNTER CARDS · V32</p><h2>相遇卡</h2><span>SWIPE UP TO DRAW · 向上滑動抽卡</span></div><div className="card-face card-front">{(current || editingActive) && <article className="mythic-card" data-testid="mythic-card" style={cardPresentationStyle}><div className="mythic-card-header"><div><span>護行之卡 · ENCOUNTER DEITY</span><h2>{encounter.artwork.zhName}</h2><small>{encounter.artwork.enName}</small></div><i>✦</i></div><div data-card-artwork><TaiwanReveal artwork={encounter.artwork} language={language}><img src={encounter.artwork.src} alt={`${encounter.artwork.zhName}・台灣神祇圖`} draggable="false" style={artworkStyle} /><div className="mythic-foil" /></TaiwanReveal></div><div className="mythic-text-panel"><div className="mythic-prompt-meta"><b>真正的你 · THE REAL YOU</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div><CardText card={encounter.card} language={language} /><BlessingText encounter={encounter} language={language} /></div></article>}</div></SwipeDeck>, 'game-card-block')}
  ...
  const appShell = (preview = false) => <main className={`app-shell v32-shell ${activeScreen}-shell${editingActive && !preview ? ' is-layout-editing' : ''}`} data-language={language} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>

...
describe('V32 layout document', () => {
  it('keeps every default setup block inside the canvas without overlaps', () => {
    const blocks = Object.values(DEFAULT_LAYOUT.screens.setup)
    for (const item of blocks) {
      expect(item.x).toBeGreaterThanOrEqual(0)
      expect(item.y).toBeGreaterThanOrEqual(0)
      expect(item.x + item.width).toBeLessThanOrEqual(LAYOUT_CANVAS.width)
      expect(item.y + item.height).toBeLessThanOrEqual(LAYOUT_CANVAS.height)
    }
    const ordered = ['header', 'hero', 'fields', 'levels', 'modes', 'begin'].map(id => DEFAULT_LAYOUT.screens.setup[id])

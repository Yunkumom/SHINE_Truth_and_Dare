...
describe('Encounter Cards setup', () => {
  ...
  it('renders the approved entrance hierarchy as V32 with Begin separated at the bottom', () => {
    render(<App />)
    expect(screen.getByText('ENCOUNTER CARDS · V32')).toBeInTheDocument()
  ...
  it('keeps advanced artwork choice collapsed and independent from the question draw', async () => {
    const user = userEvent.setup()
    render(<App />)
    const disclosure = screen.getByRole('button', { name: /Advanced deck choice/ })
    expect(disclosure).toHaveAttribute('aria-expanded', 'false')
    await user.click(disclosure)
    expect(screen.getByRole('radio', { name: /全部隨機/ })).toBeChecked()
    expect(screen.getByText(/台灣神明/)).toBeInTheDocument()
    expect(screen.getByText(/台灣星象/).closest('article')).toHaveTextContent(/規劃中|Planned/)
  ...
  it('lets another participant choose a preferred face while the question stays hidden until draw', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByRole('button', { name: /Advanced deck choice/ }))
    ...
    expect(screen.getByRole('img', { name: /媽祖・台灣神祇圖/ })).toBeInTheDocument()
    expect(screen.getByTestId('mythic-card')).toBeInTheDocument()
  })
})

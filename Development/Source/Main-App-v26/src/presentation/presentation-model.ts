...
export const PRESENTATION_STORAGE_KEY = 'encounter-presentation-v26'
...
export const DEFAULT_PRESENTATION: PresentationDocument = {
  ...
  artworkById: {},
  artworkHeight: 62,
  blessing: { fontScale: 1, lineHeight: 1.25, height: 58, padding: 5, offsetX: 0, offsetY: 0 },
}
...
export function normalizePresentation(value: unknown): PresentationDocument {
  ...
  return {
    ...
    blessing: {
      fontScale: clamp(blessing.fontScale, 1, .75, 1.6),
...
export function importPresentation(json: string): PresentationDocument {
  ...
  if (FORBIDDEN_PERSONAL_KEY.test(serialized.replace(/"(?:artworkById|schemaVersion|artworkHeight|blessing|fontScale|lineHeight|height|padding|offsetX|offsetY|zoom)"/g, ''))) throw new Error('Presentation JSON contains personal data')

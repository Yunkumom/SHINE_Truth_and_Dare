...
export default function App() {
  ...
  const editingActive = editorOpen || desktopWorkspace
  const activeScreen: LayoutScreen = desktopWorkspace || editorOpen ? editorScreen : keepsakeOpen ? 'keepsake' : playing ? 'game' : 'setup'
  ...
  const setupScreen = <section className="screen-canvas setup-canvas" aria-label="開始遊戲設定 · Game setup">
    ...
    {block('setup', 'begin', <div className="begin-dock"><button className="primary-button" aria-label={t.begin} onClick={() => { setPlaying(true); setKeepsakeOpen(false); setEditorScreen('game'); setSelectedBlock('card') }}><span>{t.begin}</span><small>LEVEL {level} · V28 · {levelLabels[level - 1][1].toUpperCase()}</small><i>→</i></button><p className="install-note">{offlineReady ? '✓ Offline ready / 可離線使用' : t.install}</p></div>, 'setup-begin-block')}
  ...
  const appShell = (preview = false) => <main className={`app-shell v28-shell ${activeScreen}-shell${editingActive && !preview ? ' is-layout-editing' : ''}`} data-language={language} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>

...
export default function App() {
  ...
  const setupScreen = <section className="screen-canvas setup-canvas" aria-label="開始遊戲設定 · Game setup">
    ...
    {block('setup', 'begin', <div className="begin-dock"><button className="primary-button" aria-label={t.begin} onClick={() => { setPlaying(true); setKeepsakeOpen(false) }}><span>{t.begin}</span><small>LEVEL {level} · V25 · {levelLabels[level - 1][1].toUpperCase()}</small><i>→</i></button><p className="install-note">{offlineReady ? '✓ Offline ready / 可離線使用' : t.install}</p></div>, 'setup-begin-block')}
  ...
  const gameScreen = <section className="screen-canvas game-canvas">
    ...
    {block('game', 'card', <SwipeDeck key={current ? `${current.card.id}-${current.artwork.id}` : 'deck'} revealed={editorOpen || revealed} onDraw={draw}><div className="card-face card-back"><span className="ornament">✦</span><p>ENCOUNTER CARDS · V25</p><h2>相遇卡</h2><span>SWIPE UP TO DRAW · 向上滑動抽卡</span></div><div className="card-face card-front">{(current || editorOpen) && <article className="mythic-card" data-testid="mythic-card"><div className="mythic-card-header"><div><span>護行之卡 · ENCOUNTER DEITY</span><h2>{encounter.artwork.zhName}</h2><small>{encounter.artwork.enName}</small></div><i>✦</i></div><div data-card-artwork><TaiwanReveal artwork={encounter.artwork} language={language}><img src={encounter.artwork.src} alt={`${encounter.artwork.zhName}・台灣神祇圖`} draggable="false" style={{ '--portrait-position': portraitObjectPosition(encounter.artwork.portraitFocus) } as React.CSSProperties} /><div className="mythic-foil" /></TaiwanReveal></div><div className="mythic-text-panel"><div className="mythic-prompt-meta"><b>真正的你 · THE REAL YOU</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div><CardText card={encounter.card} language={language} /><BlessingText encounter={encounter} language={language} /></div></article>}</div></SwipeDeck>, 'game-card-block')}
  ...
  const keepsakeScreen = <section className="screen-canvas keepsake-canvas" aria-label="Keepsake preview">
    ...
    {block('keepsake', 'artwork', <div className="keepsake-art"><img src={encounter.artwork.src} alt={`${encounter.artwork.zhName} keepsake artwork`} style={{ '--portrait-position': portraitObjectPosition(encounter.artwork.portraitFocus) } as React.CSSProperties} /></div>, 'keepsake-art-block')}
  ...
  const appShell = (preview = false) => <main className={`app-shell v25-shell ${activeScreen}-shell${editorOpen && !preview ? ' is-layout-editing' : ''}`} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>

  if (desktopWorkspace) return <div className="viewport-stage desktop-viewport"><div className="desktop-workspace" data-testid="desktop-workspace" style={{ '--workbench-scale': desktopScales.workbench, '--preview-scale': desktopScales.preview } as React.CSSProperties}><section className="desktop-workbench" aria-label="Desktop enlarged workspace"><div className="desktop-workbench-stage"><div className="desktop-workbench-canvas">{appShell()}</div></div><LayoutEditor open history={layoutHistory} onHistoryChange={setLayoutHistory} screen={editorScreen} onScreenChange={setEditorScreen} selectedBlock={selectedBlock} onSelectBlock={setSelectedBlock} onClose={() => undefined} snap={snap} onSnapChange={setSnap} docked /></section><aside className="desktop-phone-preview" aria-label="430 by 932 phone preview"><div className="desktop-phone-canvas" aria-hidden="true" inert>{appShell(true)}</div></aside></div></div>

  return <div className="viewport-stage"><div className="phone-fit-stage" style={{ '--phone-scale': phoneScale } as React.CSSProperties}>{appShell()}</div><LayoutEditor open={editorOpen} history={layoutHistory} onHistoryChange={setLayoutHistory} screen={editorScreen} onScreenChange={setEditorScreen} selectedBlock={selectedBlock} onSelectBlock={setSelectedBlock} onClose={() => setEditorOpen(false)} snap={snap} onSnapChange={setSnap} /></div>
}

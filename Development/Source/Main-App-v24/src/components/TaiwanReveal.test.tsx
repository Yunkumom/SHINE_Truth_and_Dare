...
describe('Taiwan artwork reveal', () => {
  ...
  it('reveals at 600ms and hides three seconds after release', () => {
    ...
    expect(screen.getByTestId('taiwan-locator')).toHaveStyle({
      '--taiwan-color': '#27d7d0',
      '--taiwan-accent': '#ff765f',
    })
    const coastlines = screen.getAllByTestId('taiwan-shape')
    expect(coastlines).toHaveLength(2)
    expect(coastlines[0]).toHaveClass('taiwan-coastline-glow')
    expect(coastlines[1]).toHaveClass('taiwan-coastline-crisp')
    coastlines.forEach(path => expect(path).toHaveAttribute('data-canonical-shape', 'taiwan-v24'))
    expect(screen.queryByTestId('taiwan-locator-dot')).not.toBeInTheDocument()
    expect(screen.queryByTestId('taiwan-locator-halo')).not.toBeInTheDocument()

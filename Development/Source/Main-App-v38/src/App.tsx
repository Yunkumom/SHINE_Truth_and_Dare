...
export default function App() {
  ...
  function goHome() { setPlaying(false); setKeepsakeOpen(false); setSettingsOpen(false); setArtworkAdjusterOpen(false); if (desktopMode === 'settings') { setEditorScreen('setup'); setSelectedBlock('hero') } }
  ...
  function header(screen: 'setup' | 'game') {
    return block(screen, 'header', <header className="site-header"><button className="wordmark" type="button" onClick={goHome} aria-label="Encounter Cards home"><span className="mark">✦</span><span>相遇卡 <small>ENCOUNTER CARDS · V38</small></span></button><div className="header-tools">{screen === 'setup' && <button type="button" className="card-library-trigger settings-trigger" aria-label="開啟設定 · Open settings" onClick={() => setSettingsOpen(true)}><span aria-hidden="true">⚙</span><small>設定</small></button>}{playing && <div className="font-size-control" aria-label="Font size"><button onClick={() => changeFont(-.1)}>A−</button><button onClick={() => changeFont(.1)}>A＋</button></div>}<div className="language-switch"><select className="language-select" aria-label="選擇語言 · Choose language" value={language} onChange={event => chooseLanguage(event.target.value as Language)}><option value="zh">中文</option><option value="en">English</option><option value="bilingual">中文／English</option></select></div></div></header>, 'header-block')
  }
  ...
  return <div className="viewport-stage v38-mobile-stage"><div className="phone-fit-stage" style={{ '--phone-scale': phoneScale } as React.CSSProperties}>{appShell()}</div></div>
}

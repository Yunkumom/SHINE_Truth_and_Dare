...
const css = readFileSync(join(process.cwd(), 'src/styles/v31-layout.css'), 'utf8')

describe('v31 setup visual contract', () => {
  it('owns one warm milk-tea palette without dark setup surfaces', () => {
    ...
    expect(css).toMatch(/\.v31-shell \.setup-canvas\{[^}]*--setup-milk-tea/)
    expect(css).toMatch(/\.v31-shell \.setup-canvas \.site-header\{[^}]*--setup-cream/)
  })

  it('reserves an in-flow title row before both setup grids', () => {
    expect(css).toMatch(/\.v31-shell \.fields-card \.panel-heading>b\{[^}]*min-height:[^;}]+[^}]*line-height:[^;}]+[^}]*overflow:visible/)
    expect(css).toMatch(/\.fields-card label>span\{[^}]*white-space:normal/)
    expect(css).toMatch(/\.v31-shell \.setup-card legend\{[^}]*display:block[^}]*min-height:[^;}]+[^}]*line-height:[^;}]+[^}]*white-space:normal/)
    expect(css).toMatch(/\.v31-shell \.setup-card legend\{[^}]*position:static/)
    expect(css).toMatch(/\.v31-shell \.setup-levels-block \.level-grid,\.v31-shell \.setup-modes-block \.mode-grid\{[^}]*margin-top:/)

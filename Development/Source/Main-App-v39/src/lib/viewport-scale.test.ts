...
import { DESKTOP_SAFE_MARGIN, PHONE_HEIGHT, PHONE_WIDTH, calculateDesktopScales, calculatePhoneLayout, calculatePhoneScale } from './viewport-scale'

describe('v18 phone-shell scale policy', () => {
  it('keeps the complete mobile canvas visible on a narrower phone', () => {
    expect(calculatePhoneScale({ width: 390, height: 844 })).toBeCloseTo(390 / PHONE_WIDTH)
  })

  it('fits the complete canvas to a short mobile viewport', () => {
    expect(calculatePhoneScale({ width: PHONE_WIDTH, height: 780 })).toBeCloseTo(780 / PHONE_HEIGHT)
  })

  it('keeps the mobile 430 × 932 contract at 1× when it fits', () => {
  ...
  it('fits the centre and smaller right preview inside the compact 1366 by 768 studio', () => {
    const scales = calculateDesktopScales(768)
    expect(scales.workbench).toBeCloseTo(.76, 2)
    expect(scales.preview).toBe(.62)
    expect(scales.workbench).toBeGreaterThan(scales.preview)
    expect(scales.workbench * PHONE_HEIGHT).toBeLessThanOrEqual(768 - 60)
  })
})

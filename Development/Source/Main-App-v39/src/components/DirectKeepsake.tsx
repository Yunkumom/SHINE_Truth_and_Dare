...
export default function DirectKeepsake({ language, artworks, collections, blessings, onBack, onStatus }: DirectKeepsakeProps) {
  ...
  async function download() {
    ...
    setBusy(true)
    try {
      const result = await downloadDirectKeepsake({ imageSrc, imageName, blessing: blessingText, language })
  ...
  return <section className="direct-keepsake-canvas" aria-label="直接製作紀念卡 · Create a keepsake">
    ...
    <div className="direct-keepsake-content">
      <article className="direct-keepsake-preview">
        ...
        <div className="direct-keepsake-image"><img src={imageSrc} alt={previewAlt} /></div>
    ...
    {pickerOpen && <div className="direct-artwork-picker" role="dialog" aria-label="選擇紀念卡圖片">
      ...
      <ArtworkPicker artworks={artworks} collections={collections} selectedArtworkId={uploaded ? undefined : artwork.id} initialCollectionId={artwork.collectionId} onSelect={value => { setArtwork(value); setUploaded(previous => { if (previous) URL.revokeObjectURL(previous.url); return null }); setPickerOpen(false) }} />
    </div>}
  </section>
}

...
const initialManagerState: SessionQuestionManagerState = { disabledQuestionIds: [], customQuestions: [], selectedQuestionId: null, showRealYou: true, showQuestion: true }
...
export default function App() {
  ...
  const [experience, setExperience] = useState<'encounter' | 'truth-dare'>('encounter')
  ...
  const beginLabel = experience === 'truth-dare'
    ? language === 'en' ? 'Begin Truth or Dare' : '開始真心話大冒險 · Begin'
    : t.begin
  ...
  useEffect(() => {
    const updateScale = () => {
      ...
      setDesktopWorkspace(window.innerWidth >= 1100)
  ...
  function draw(artworkId?: string) {
    const question = resolveManagedQuestion({ base: cards, custom: questionManager.customQuestions, disabledQuestionIds: questionManager.disabledQuestionIds, selectedQuestionId: questionManager.selectedQuestionId, level, mode, previousId: current?.card.id })
  ...
  function chooseExperience(value: 'encounter' | 'truth-dare') { setExperience(value); setMode('random'); goToSetup() }
  ...
  function header(screen: 'setup' | 'game') {
    return block(screen, 'header', <header className="site-header"><button className="wordmark" type="button" onClick={goToModeHome} aria-label="返回模式選擇 · Back to modes"><span className="mark">✦</span><span>相遇卡 <small>ENCOUNTER CARDS · V39</small></span></button><div className="header-tools">{screen === 'setup' && <button type="button" className="card-library-trigger settings-trigger" aria-label="開啟設定 · Open settings" onClick={() => setSettingsOpen(true)}><span className="menu-icon" aria-hidden="true"><i /><i /><i /></span></button>}{playing && <div className="font-size-control" aria-label="Font size"><button onClick={() => changeFont(-.1)}>A−</button><button onClick={() => changeFont(.1)}>A＋</button></div>}<div className="language-switch"><select className="language-select" aria-label="選擇語言 · Choose language" value={language} onChange={event => chooseLanguage(event.target.value as Language)}><option value="zh">中文</option><option value="en">English</option><option value="bilingual">中文／English</option></select></div></div></header>, 'header-block')
  }
  ...
  const artworkStyle = {
    ...
    '--artwork-offset-x': `${artworkPresentation.offsetX}%`,
    '--artwork-offset-y': `${artworkPresentation.offsetY}%`,
  ...
  function updateArtworkPresentation(value: typeof artworkPresentation) {
    setPresentation(document => normalizePresentation({ ...document, artworkById: { ...document.artworkById, [encounter.artwork.id]: value } }))
  }
  ...
  const setupScreen = <section className="screen-canvas setup-canvas" aria-label="開始遊戲設定 · Game setup">
    ...
    {block('setup', 'begin', <div className="begin-dock"><button className="primary-button" aria-label={beginLabel} onClick={() => { setCandidateArtworks(createArtworkCandidates(ARTWORK_COLLECTIONS, ALL_ARTWORKS, artworkPreference)); setRevealed(false); setPlaying(true); setKeepsakeOpen(false); setSettingsOpen(false); setEditorScreen('game'); setSelectedBlock('card') }}><span>{beginLabel}</span><small>LEVEL {level} · V39 · {experience === 'truth-dare' ? 'TRUTH OR DARE' : levelLabels[level - 1][1].toUpperCase()}</small><i>→</i></button><p className="install-note">{offlineReady ? '✓ Offline ready / 可離線使用' : t.install}</p></div>, 'setup-begin-block')}
  ...
  const gameScreen = <section className="screen-canvas game-canvas" aria-hidden={settingsOpen || artworkAdjusterOpen || undefined} inert={settingsOpen || artworkAdjusterOpen || undefined}>
    {header('game')}
    {block('game', 'toolbar', <div className="game-toolbar"><button onClick={goToSetup}>← Setup</button><div className="game-meta"><span>LEVEL {level}</span><span>{mode.toUpperCase()}</span><span>42 ARTWORKS</span></div></div>, 'game-toolbar-block')}
    {block('game', 'card', <SwipeDeck key={current ? `${current.card.id}-${current.artwork.id}` : 'deck'} revealed={editingActive || revealed} onDraw={draw}><div className="card-face card-back"><span className="ornament">✦</span><p>ENCOUNTER CARDS · V39</p><h2>相遇卡</h2><span>SWIPE UP TO DRAW · 向上滑動抽卡</span></div><div className="card-face card-front">{(current || editingActive) && <article className="mythic-card" data-testid="mythic-card" style={cardPresentationStyle}><div className="mythic-card-header"><div><span>護行之卡 · ENCOUNTER GUARDIAN</span><h2>{encounter.artwork.zhName}</h2><small>{encounter.artwork.enName}</small>{encounter.artwork.featureLabel && <small className="artwork-feature-note">台灣特色 · {localizedFeature(encounter.artwork, language)}</small>}</div><i>✦</i></div><div data-card-artwork><TaiwanReveal artwork={encounter.artwork} language={language}><img src={encounter.artwork.src} alt={`${encounter.artwork.zhName}・藏有台灣輪廓的卡面`} draggable="false" style={artworkStyle} /><div className="mythic-foil" /></TaiwanReveal></div><div className="mythic-text-panel">{questionManager.showRealYou && <div className="mythic-prompt-meta"><b>真正的你 · THE REAL YOU</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div>}{questionManager.showQuestion && <CardText card={encounter.card} language={language} />}<BlessingText encounter={encounter} language={language} /></div></article>}</div></SwipeDeck>, 'game-card-block')}
    {!revealed && !editingActive && <section className="artwork-candidate-fan artwork-tools-combined" aria-label="Choose a favorite card face"><div><b>挑一張喜歡的卡面・調整照片</b><small>CHOOSE &amp; ADJUST</small><button type="button" className="inline-artwork-reselect" onClick={() => setSettingsOpen(true)}>抽卡設定</button></div><div>{candidateArtworks.map(artwork => <button type="button" key={artwork.id} aria-label={`選擇 ${artwork.zhName} · Choose ${artwork.enName} · ${artwork.id}`} onClick={() => draw(artwork.id)}><img src={artwork.src} alt="" /><span>{artwork.zhName}</span></button>)}</div></section>}
    {revealed && current && !editingActive && <section className="artwork-control-panel" aria-label="Choose and adjust card artwork"><div className="artwork-control-copy"><img src={current.artwork.src} alt="" /><span><b>挑一張喜歡的卡面・調整照片</b><small>{current.artwork.zhName}</small></span></div><button type="button" onClick={() => setSettingsOpen(true)}>重新選擇</button><button type="button" onClick={() => setArtworkAdjusterOpen(true)}>調整照片</button></section>}
  ...
  const keepsakeScreen = <section className="screen-canvas keepsake-canvas" aria-label="Keepsake preview">
    ...
    {questionManager.showQuestion && block('keepsake', 'question', <div className="keepsake-question">{questionManager.showRealYou && <div><b>⌖ 想去的地方</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div>}<CardText card={encounter.card} language={language} /></div>, 'keepsake-question-block')}
  ...
  const specialMobileScreen = !desktopWorkspace && mobileDestination === 'home'
    ? <ModeHome language={language} onChooseEncounter={() => chooseExperience('encounter')} onChooseKeepsake={() => { setPlaying(false); setKeepsakeOpen(false); setMobileDestination('keepsake-maker') }} onChooseTruthOrDare={() => chooseExperience('truth-dare')} />
  ...
  const appShell = (preview = false) => <main className={`app-shell v32-shell v33-shell v34-shell v35-shell v37-shell v39-shell ${shellName}-shell${editingActive && !preview ? ' is-layout-editing' : ''}`} data-language={language} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && !specialMobileScreen && <><MobileSettings open={settingsOpen} onClose={() => setSettingsOpen(false)} level={level} mode={mode} onLevelChange={chooseLevel} onModeChange={setMode} artworks={ALL_ARTWORKS} collections={ARTWORK_COLLECTIONS} artworkPreference={artworkPreference} onArtworkPreferenceChange={setArtworkPreference} questions={cards} manager={questionManager} onManagerChange={setQuestionManager} savedArtworkIds={Object.keys(presentation.artworkById)} onClearArtworkPosition={clearArtworkPresentation} />{artworkAdjusterOpen && <ArtworkAdjuster open artwork={encounter.artwork} value={artworkPresentation} onChange={updateArtworkPresentation} onClose={() => setArtworkAdjusterOpen(false)} />}</>}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>
  ...
  const desktopModeTabs = <div className="desktop-mode-switch desktop-mode-bookmark-tabs" role="group" aria-label="Desktop mode"><button type="button" aria-label="Settings mode" aria-pressed={desktopMode === 'settings'} className={desktopMode === 'settings' ? 'active' : ''} onClick={() => setDesktopMode('settings')}><span>設定</span><small>SETTINGS</small></button><button type="button" aria-label="Test mode" aria-pressed={desktopMode === 'test'} className={desktopMode === 'test' ? 'active' : ''} onClick={() => { setDesktopMode('test'); setDirectManipulation(false) }}><span>測試</span><small>TEST</small></button></div>
  ...
  if (desktopWorkspace) return <div className="viewport-stage desktop-viewport">
    {desktopMode === 'settings'
      ? <div className="desktop-workspace" data-testid="desktop-workspace" style={{ '--workbench-scale': desktopScales.workbench, '--preview-scale': desktopScales.preview } as React.CSSProperties}>
          <aside className="desktop-editor-rail" data-testid="desktop-editor-rail"><LayoutEditor open history={layoutHistory} onHistoryChange={setLayoutHistory} screen={editorScreen} onScreenChange={setEditorScreen} selectedBlock={selectedBlock} onSelectBlock={setSelectedBlock} onClose={() => undefined} snap={snap} onSnapChange={setSnap} directManipulation={directManipulation} onDirectManipulationChange={setDirectManipulation} docked {...editorProps} /></aside>
          <div className="desktop-center-column"><div className="desktop-mode-bookmarks" data-testid="desktop-mode-bookmarks">{desktopModeTabs}</div><section className="desktop-workbench-stage desktop-center-stage" data-testid="desktop-center-stage" aria-label="Desktop enlarged workspace"><div className="desktop-device-frame desktop-workbench-device"><div className="desktop-workbench-canvas">{appShell()}</div></div></section></div>
          <aside className="desktop-phone-preview" aria-label="78 by 163.4 millimeter iPhone Pro Max preview"><div className="desktop-device-frame desktop-preview-device"><div className="desktop-phone-canvas" aria-hidden="true" inert>{appShell(true)}</div></div></aside>
        </div>
  ...
  return <div className="viewport-stage v39-mobile-stage"><div className="phone-fit-stage" style={{ '--phone-scale': phoneScale } as React.CSSProperties}>{appShell()}</div></div>
}

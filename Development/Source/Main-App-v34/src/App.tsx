...
export default function App() {
  ...
  const appShell = (preview = false) => <main className={`app-shell v32-shell v33-shell v34-shell ${activeScreen}-shell${editingActive && !preview ? ' is-layout-editing' : ''}`} data-language={language} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>

...
export default function EditableBlock({ id, block, editing, selected, directManipulation = false, canvasScale = 1, snap = false, onSelect, onChange, children, className = '' }: EditableBlockProps) {
  const gesture = useRef<DirectGesture | null>(null)

  function beginMove(event: PointerEvent<HTMLDivElement>) {
    const target = event.target as HTMLElement
    if (!editing || !directManipulation || target.closest('[data-resize-handle],button,input,select,textarea,label,a,[role="button"],[data-no-layout-drag]')) return
  ...
  return <div
    className={`editable-block ${className}${editing ? ' is-editing' : ''}${selected ? ' is-selected' : ''}${directManipulation ? ' is-direct-manipulation' : ''}`}
  ...
  >
    {children}
    {editing && selected && directManipulation && <button
      type="button"

...
import type { ArtworkPreference, Card, EncounterComposition, Language, Level, Mode, ParticipantExchange, QuestionPreference } from './types'
...
export default function App() {
  ...
  const gameScreen = <section className="screen-canvas game-canvas">
    {header('game')}
    {block('game', 'toolbar', <div className="game-toolbar"><button onClick={goHome}>← Setup</button><div className="game-meta"><span>LEVEL {level}</span><span>{mode.toUpperCase()}</span><span>30 ARTWORKS</span></div></div>, 'game-toolbar-block')}
    {block('game', 'card', <SwipeDeck key={current ? `${current.card.id}-${current.artwork.id}` : 'deck'} revealed={editingActive || revealed} onDraw={draw}><div className="card-face card-back"><span className="ornament">✦</span><p>ENCOUNTER CARDS · V33</p><h2>相遇卡</h2><span>SWIPE UP TO DRAW · 向上滑動抽卡</span></div><div className="card-face card-front">{(current || editingActive) && <article className="mythic-card" data-testid="mythic-card" style={cardPresentationStyle}><div className="mythic-card-header"><div><span>護行之卡 · ENCOUNTER GUARDIAN</span><h2>{encounter.artwork.zhName}</h2><small>{encounter.artwork.enName}</small></div><i>✦</i></div><div data-card-artwork><TaiwanReveal artwork={encounter.artwork} language={language}><img src={encounter.artwork.src} alt={`${encounter.artwork.zhName}・藏有台灣輪廓的卡面`} draggable="false" style={artworkStyle} /><div className="mythic-foil" /></TaiwanReveal></div><div className="mythic-text-panel"><div className="mythic-prompt-meta"><b>真正的你 · THE REAL YOU</b><span>L{encounter.card.level} · {encounter.card.mode.toUpperCase()}</span></div><CardText card={encounter.card} language={language} /><BlessingText encounter={encounter} language={language} /></div></article>}</div></SwipeDeck>, 'game-card-block')}
  ...
  const appShell = (preview = false) => <main className={`app-shell v32-shell v33-shell ${activeScreen}-shell${editingActive && !preview ? ' is-layout-editing' : ''}`} data-language={language} style={{ '--font-scale': fontScale } as React.CSSProperties}>{screen}{!preview && <p className="sr-only" aria-live="polite">{status}</p>}</main>

...
describe('v18 phone-shell scale policy', () => {
  ...
  it('reserves the desktop mode switch and keeps the complete 932px device inside a 720px viewport', () => {
    const scales = calculateDesktopScales(720)
    expect(scales.workbench * PHONE_HEIGHT).toBeLessThanOrEqual(720 - 88)
    expect(scales.preview * PHONE_HEIGHT).toBeLessThanOrEqual(720 - 88)
  })
})

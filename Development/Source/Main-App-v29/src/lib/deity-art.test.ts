...
describe('v28 deity card artwork', () => {
  ...
  it('uses the wide portrait-safe Baosheng apothecary source and its aligned safe-zone locator', () => {
    const artwork = DEITY_ART.find(item => item.id === 'baosheng-apothecary')
    expect(artwork?.src).toContain('baosheng-apothecary-wide-taiwan-safe-v28')
    expect(artwork?.portraitFocus).toEqual({ x: 50, y: 34 })
    expect(artwork?.taiwanHotspot).toMatchObject({ x: 52, y: 68, scale: .38, rotation: 8 })
  })
})

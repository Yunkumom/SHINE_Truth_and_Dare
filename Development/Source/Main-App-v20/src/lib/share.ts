...
export async function createCardPng(composition: EncounterComposition, language: Language): Promise<File> {
  ...
  context.textAlign = 'right'; context.fillStyle = '#8d4d32'; context.font = '600 21px system-ui'; context.fillText(`LEVEL ${card.level} · ${card.mode.toUpperCase()} · V20`, question.x + question.width - 38, question.y + 48); context.textAlign = 'left'
  ...
  return new File([blob], `encounter-card-v20-${card.id}-${artwork.id}.png`, { type: 'image/png' })
}
...
export async function deliverCardFile(file: File, target: ShareTarget = navigator): Promise<'shared' | 'downloaded' | 'cancelled'> {
  if (target.share && (!target.canShare || target.canShare({ files: [file] }))) {
    try { await target.share({ files: [file], title: 'Encounter Cards v20' }); return 'shared' }

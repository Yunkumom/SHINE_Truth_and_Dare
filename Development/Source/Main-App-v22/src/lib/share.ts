...
export async function createCardPng(composition: EncounterComposition, language: Language, participants: ParticipantExchange[] = [], layout: LayoutDocument = DEFAULT_LAYOUT): Promise<File> {
  ...
  context.textAlign = 'right'; context.fillStyle = '#8d4d32'; context.font = '600 20px system-ui'; context.fillText(`L${card.level} · ${card.mode.toUpperCase()} · V22`, question.x + question.width - 38, question.y + 43); context.textAlign = 'left'
  ...
  return new File([blob], `encounter-card-v22-${card.id}-${artwork.id}.png`, { type: 'image/png' })
}
...
export async function deliverCardFile(file: File, target: ShareTarget = navigator): Promise<'shared' | 'downloaded' | 'cancelled'> {
  if (target.share && (!target.canShare || target.canShare({ files: [file] }))) {
    try { await target.share({ files: [file], title: 'Encounter Cards v22' }); return 'shared' }

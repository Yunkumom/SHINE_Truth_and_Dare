...
describe('V32 deity card artwork', () => {
  ...
  it('registers two local variants for every one of nine deities', () => {
    ...
    expect(DEITY_ART.every(artwork => artwork.src.match(/(?:^data:image\/(?:webp|png)|\.(?:webp|png)$)/) && artwork.hiddenTaiwan)).toBe(true)
  })
  ...
  it('keeps every hidden Taiwan locator inside the visible card crop', () => {
    ...
    expect(DEITY_ART.every(artwork => artwork.src.includes('safe-v32'))).toBe(true)
    expect(new Set(DEITY_ART.map(artwork => artwork.src)).size).toBe(18)
    expect(DEITY_ART.every(artwork => artwork.portraitFocus.x === 50 && artwork.portraitFocus.y <= 40)).toBe(true)
  })

  it('uses the regenerated portrait-safe Baosheng apothecary master and aligned central locator', () => {
    const artwork = DEITY_ART.find(item => item.id === 'baosheng-apothecary')
    expect(artwork?.src).toContain('baosheng-apothecary-safe-v32')
    expect(artwork?.portraitFocus).toEqual({ x: 50, y: 20 })
    expect(artwork?.taiwanHotspot).toMatchObject({ x: 51, y: 76, scale: .72, rotation: 0 })
  })
})

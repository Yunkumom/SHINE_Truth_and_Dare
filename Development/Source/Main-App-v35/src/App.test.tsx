...
describe('Encounter Cards setup', () => {
  ...
  it('renders the approved entrance hierarchy as V35 with Begin separated at the bottom', () => {
    render(<App />)
    expect(screen.getByText('ENCOUNTER CARDS · V35')).toBeInTheDocument()
  ...
  it('opens the entrance card library and keeps artwork choice independent from questions', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByRole('button', { name: '開啟卡庫 · Open card library' }))
    ...
    expect(screen.queryByRole('dialog', { name: '卡庫 · Card library' })).not.toBeInTheDocument()
    const disclosure = screen.getByRole('button', { name: /Exact choice/ })
    expect(disclosure).toHaveTextContent(/已指定|CUSTOM/)
    await user.click(disclosure)
    expect(screen.getByRole('button', { name: '隨機問題' })).toHaveAttribute('aria-pressed', 'true')
  })

      ...
      <main className={`app-shell ${playing ? 'game-shell' : 'landing-shell'}`} style={{ '--font-scale': fontScale } as React.CSSProperties}>
        <header className="site-header">
          <button className="wordmark" type="button" onClick={() => setPlaying(false)} aria-label="Encounter Cards home">
            ...
            <span>相遇卡 <small>ENCOUNTER CARDS · V21</small></span>
          </button>
        ...
        {!playing ? <section className="hero" id="top">
          ...
          <div className="setup-panel" aria-label="開始遊戲設定 · Game setup">
            ...
            <button className="primary-button" aria-label={t.begin} onClick={() => setPlaying(true)}><span>{t.begin}</span><small>LEVEL {level} · V21 · {levelLabels[level - 1][1].toUpperCase()}</small><i>→</i></button>
          ...
          <div className={`card-stack ${current ? 'has-card' : ''}`}>
            ...
            <div className={`encounter-card ${revealed ? 'is-flipped' : ''}`} tabIndex={0}
              ...
              <div className="card-face card-back"><span className="ornament">✦</span><p>ENCOUNTER CARDS · V21</p><h2>相遇卡</h2><span>DRAG TO DRAW · 滑動抽卡</span></div>
              <div className="card-face card-front">{current && <article className="mythic-card" data-testid="mythic-card">
                ...
                <TaiwanReveal artwork={current.artwork} language={language}><img src={current.artwork.src} alt={`${current.artwork.zhName}・台灣神祇圖`} draggable="false" /><div className="mythic-foil" /></TaiwanReveal>

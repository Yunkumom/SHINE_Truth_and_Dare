...
Corrective deployment evidence from 2026-08-19: the deployed mobile Truth-or-Dare setup still exposed the legacy encounter headline, lead, setup label, keepsake blessing label, and direct/export title after the earlier rollout. The correction replaces the setup headline with `一起玩一題吧。`, uses `準備開始 · Get ready`, keeps the gameplay brand `TRUTH OR DARE`, and changes the keepsake label to `給今天的祝福 · BLESSING`; direct and post-draw PNG exports no longer render `相遇紀念卡` or `給這次相遇的祝福`. A new focused regression passed, lint passed with zero errors and ten existing `<img>` warnings, and the production build emitted `index-bxiyAIM-.js` with the existing `index-Dd_IiPti.css`. Firebase released all 51 files again at `https://shine-truth-or-dare-dev.web.app`. A fresh 430 × 932 public browser run verified the compact home, 418 px direct card at ratio 0.715916 with `object-fit: cover`, friendly setup copy, English-only card back, question-only reveal, post-draw question/keywords/contact editor, 418 px post-draw card, no horizontal overflow, and zero captured warning/error logs. This remains a preview-only deployment; v46 stays `latestVerified` and no promotion occurred.

2026-08-19 修正部署證據：先前發布後，已部署的手機 Truth-or-Dare 設定仍顯示舊的相遇標題、說明、設定標籤、紀念卡祝福標籤與直接／匯出標題。這次修正把設定標題改為 `一起玩一題吧。`、使用 `準備開始 · Get ready`、保持遊戲品牌為 `TRUTH OR DARE`，並把紀念卡標籤改為 `給今天的祝福 · BLESSING`；直接與抽卡後 PNG 匯出也不再繪製 `相遇紀念卡` 或 `給這次相遇的祝福`。新增的聚焦回歸已通過，lint 以零錯誤及十項既有 `<img>` 警告通過，production build 產生 `index-bxiyAIM-.js` 並沿用 `index-Dd_IiPti.css`。Firebase 已再次發布全部 51 個檔案至 `https://shine-truth-or-dare-dev.web.app`。最新 430 × 932 公開瀏覽器檢查已驗證精簡首頁、418 px 且比例 0.715916 並使用 `object-fit: cover` 的直接卡片、友善設定文案、純英文卡背、只顯示題目的翻面、抽卡後題目／關鍵字／聯絡編輯器、418 px 抽卡後卡片、沒有水平溢出，以及零筆 warning／error log。這仍只是 preview 部署；v46 維持 `latestVerified`，未執行 promotion。
